...
class MAXIM:
    def __init__(self, modelPath, has_target,input_file,target_filename,geometric_ensemble,save_img,output_file ):
        ...
        self._model = self._create_maxim_model()
    ...
    def _preprocess(self):
        ...
        if not self._is_image_file(self._input_file):
        ...
        input_img = self._convert_img_shape(input_img)

        if self._geometric_ensemble:
            input_img = self._augment_image(input_img, args.ensemble_times)
        else:
    ...
    def _postprocess(self, preds, height, width):
        ...
        if self._geometric_ensemble:
            preds = self._deaugment_image(preds, args.ensemble_times)
        else:
        ...
        # save files
        if self._save:
            # os.makedirs(os.self._output_file, exist_ok=True)
            self._save_img(preds, self._output_file)

        return preds, psnr

    
    def _recover_tree(self,keys, values):
        ...
        """
        tree = {}
        sub_trees = self._collections.defaultdict(list)
        ...
        for k, kv_pairs in sub_trees.items():
            ...
            tree[k] = self._recover_tree(k_subtree, v_subtree)
        return tree
    ...
    def _calculate_psnr(self,img1, img2, crop_border, test_y_channel=False):
        ...
        if test_y_channel:
            img1 = self._to_y_channel(img1)
            img2 = self._to_y_channel(img2)
    ...
    def _rgb2ycbcr(self,img, y_only=False):
    ...
    def _to_y_channel(self,img):
        ...
        if img.ndim == 3 and img.shape[2] == 3:
            img = self._rgb2ycbcr(img, y_only=True)
            img = img[..., None]
        return img * 255.


    def _augment_image(sef,image, times=8):
    ...
    def _deaugment_image(self,images, times=8):
    ...
    def _convert_img_shape(self,image):
