...
class nanodet():
    def __init__(self, prob_threshold=0.35, iou_threshold=0.6):
        ...
        for i in range(len(self.strides)):
            anchors = self._make_grid((int(self.image_shape[0] / self.strides[i]), int(self.image_shape[1] / self.strides[i])), self.strides[i])
    ...
    def get_bboxes_single(self, cls_scores, bbox_preds, scale_factor, rescale=False):
        ...
        for stride, cls_score, bbox_pred, anchors in zip(self.strides, cls_scores, bbox_preds, self.anchors_mlvl):
            ...
            bboxes = self.distance2bbox(anchors, bbox_pred, max_shape=self.image_shape)
        ...
        confidences = np.max(scores_mlvl, axis=1)  ####max_class_confidence

        indices = cv2.dnn.NMSBoxes(bboxes_wh.tolist(), confidences.tolist(), self.prob_threshold, self.iou_threshold)
        if len(indices)>0:
            bboxes_mlvl = bboxes_mlvl[indices[:]]
            confidences = confidences[indices[:]]
            classIds = classIds[indices[:]]
            return bboxes_mlvl, confidences, classIds
        else:
